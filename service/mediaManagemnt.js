...
export async function uploadImage(file) {
  ...
  try {
    ...
    const message =
      error?.response?.data?.detail?.[0]?.msg || "Failed to Upload Image";
...
export async function deleteMedia(mediaType, imageUrl) {
  const payload = {
    mediaType: mediaType,
    imageUrl: imageUrl,
  };
  try {
    return await axiosInstance.delete("/media/remove/media",{
      data: payload, 
    });
  } catch (error) {
    throw new Error(error.response?.data || "Failed to to Delete Media");
  }
}
...
export async function uploadPDF(file, onProgress) {
  ...
  try {
    ...
    const message =
      error?.response?.data?.detail?.[0]?.msg || "Failed to Upload PDF";
    throw new Error(message);
  }
}
