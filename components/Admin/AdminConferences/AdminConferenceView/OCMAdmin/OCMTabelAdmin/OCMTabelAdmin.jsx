...
import {patchOCM, deleteOCM,updateOCMStatus} from "@/service/AdminConfernecePages/confernce";
export default function SpeakerTabelAdmin({
  ...
  const handleStatusChange = async (newStatus, id) => {
    try {
      const response = await updateOCMStatus(selectedConferenceID, {ocmId:id,status:newStatus});
  ...
  const handleDelete = async (id) => {
    try {
     ...
     const response=await deleteOCM(selectedConferenceID,{ocmId:id});
           if(response.status !== 200) {
  ...
  const handleModel = (type, data = null) => {
    ...
    const componentsMap = {
      ...
      edit: {
        ...
        content: (
          <Edit
            formData={data} selectedConferenceID={selectedConferenceID}
                ...
                <tr key={i}>
                  ...
                  <td className="p-3  table-data ">
                    ...
                    <InputSwitch
                      ...
                      onChange={(e) => handleStatusChange(e.value, element.ocmId)}
                      style={{ scale: "0.7" }}
                    />
                  </td>
                  <td className="p-3 table-data ">
                    <div className="d-flex gap-1  justify-content-center flex-nowrap">
                      ...
                      <button
                        className="btn btn-outline-secondary rounded"
                        onClick={() => confirmDelete(element.ocmId)}
                      >
...
function Edit({ formData,selectedConferenceID, toast, fetchData, setIsVisible }) {
  ...
  const submitSpeaker = async (data) => {
    ...
    try {
      ...
      console.log(data);
      const response = await patchOCM(selectedConferenceID,payLoad,formData.ocmId);
