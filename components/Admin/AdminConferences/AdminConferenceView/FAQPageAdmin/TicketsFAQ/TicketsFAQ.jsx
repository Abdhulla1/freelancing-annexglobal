...
export default function TicketsFAQ({   selectedConferenceID,
  toast,
  ticketsFAQ,
  fetchConfernceData}) {
  ...
  const handleSidebar = (type, data = null) => {
    const componentsMap = {
      view: {
        header: "View TicketsFAQ",
        content: <View data={data} />,
      },
      edit: {
        header: "Edit TicketsFAQ",
      ...
      try {
        ...
        if (response.status === 200) {
          toast.current?.show({
            severity: "success",
            summary: "Updated",
            detail: response.data?.detail?.[0]?.msg || "TicketsFAQ updated successfully",
          });
          fetchConfernceData();
          setIsVisible(false);
        } else {
