...
import { updatefaq } from "@/service/AdminConfernecePages/confernce"; // adjust import
import { Button } from "primereact/button";

export default function  AttendingEventFAQ({   selectedConferenceID,
  toast,
  attendingEventFAQ,
  fetchConfernceData}) {
  ...
  const actualData = attendingEventFAQ || [];
  ...
  const handleSidebar = (type, data = null) => {
    const componentsMap = {
      view: {
        header: "View Attending Event FAQ",
        content: <View data={data} />,
      },
      edit: {
        header: "Edit Attending Event FAQ",
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const payload = {
          faqType: "attendingEvent",
          title: values.title,
          content: values.content,
        };

        const response = await updatefaq( selectedConferenceID,payload, data.faqId);

        if (response.status === 200) {
          toast.current?.show({
            severity: "success",
            summary: "Updated",
            detail: response.data?.detail?.[0]?.msg || "GeneralFAQ updated successfully",
          });
          fetchConfernceData();
          setIsVisible(false);
        } else {
