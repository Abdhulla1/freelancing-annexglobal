...
import { deleteEssentialInnovation,patchEssentialInnovation} from "@/service/AdminConfernecePages/confernce";
...
{  selectedConferenceID,
  ...
  const handleDelete = async (topicId) => {
    try {
      ...
      const response=await deleteEssentialInnovation(selectedConferenceID,payload);
      ...
      {essentialInnovation.length=== 0 ? (
        ...
        <>
          <table className="tabel w-100  table-striped-columns">
            ...
            <tbody>
              {essentialInnovation.slice(first, first + rows).map((element, i) => (
                <tr key={i}>
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const response = await patchEssentialInnovation(selectedConferenceID,payload,data.topicId);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const response = await patchEssentialInnovation(selectedConferenceID,payload);
