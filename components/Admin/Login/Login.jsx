        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    toast.current.show({ severity: 'error', summary: 'Login Failed', detail: data.message || 'Login failed.', life: 3000 });
                }
            } catch (error) {
                toast.current.show({
                    severity: 'error',
    ...
    const forgotFormik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                if (data.success) {
                    toast.current.show({ severity: 'success', summary: 'Reset Link Sent', detail: 'Please check your email for the password reset link.', life: 3000 });
                    setIsForgotPasswordOpen(false);
                } else {
                    toast.current.show({ severity: 'error', summary: 'Request Failed', detail: data.message || 'Failed to send reset link. Please try again.', life: 3000 });
                }
            } catch (err) {
                console.error("Forgot password error:", err);
                toast.current.show({ severity: 'error', summary: 'Error', detail: 'Something went wrong while requesting password reset.', life: 3000 });
            }
        },
    });
                            ...
                            <form className="w-50 mt-4" onSubmit={forgotFormik.handleSubmit}>
                                <div className="mb-4">
                                    ...
                                    <input
                                        ...
                                        required
                                    />
                                    {forgotFormik.touched.resetEmail && forgotFormik.errors.resetEmail && (
                                        <div className="text-danger">{forgotFormik.errors.resetEmail}</div>
                                    )}
                                </div>
                                <button
                                    type="submit"
